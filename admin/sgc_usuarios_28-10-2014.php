				...
				<table class="da-table" '.$id.'>
					<thead>
						<tr>
							...
							<th><b><span lang="es">Cargo</span></b></th>
					  ...
					  if($num>0){
							while($regi = $res->fetch_array(MYSQLI_ASSOC)){
								...
								echo'>
								   ...
								   echo'</td>
								   		<td><span lang="es">'.$regi['cargo'].'</span></td>
										<td><span lang="es">'.$regi['activado'].'</span></td>

										<td style="text-align:center; width:100px;">';
										   echo'
										   <ul class="action_user">
											<li><a href="?l=usuarios&idusuario='.base64_encode($regi['id_usuario']).'&editar=v" class="edit da-tooltip-s" title="<span lang=\'es\'>Editar</span>"></a></li>
											<li><a href="?l=usuarios&idusuario='.base64_encode($regi['id_usuario']).'&cpass=v" class="privacidad da-tooltip-s" title="<span lang=\'es\'>Cambiar Contrase&ntilde;a</span>"></a></li>';
											if($tipo_user==md5('ROOT')){
											   echo'<li style="margin-left:5px;"><a href="?l=usuarios&idusuario='.base64_encode($regi['id_usuario']).'&reset=v" class="resetear da-tooltip-s" title="<span lang=\'es\'>Resetear Contrase&ntilde;a</span>"></a></li>';
											   if($regi['activado']=='activo'){
												  echo'<li style="margin-left:5px;"><a href="#" class="darbaja da-tooltip-s activar_user" id="'.base64_encode($regi['id_usuario']).'|dar baja" title="<span lang=\'es\'>dar baja</span>"></a></li>';
											   }elseif($regi['activado']=='inactivo'){
												  echo'<li style="margin-left:5px;"><a href="#" class="daralta da-tooltip-s activar_user" id="'.base64_encode($regi['id_usuario']).'|dar alta" title="<span lang=\'es\'>dar alta</span>"></a></li>';
											   }
											}
	...
	if($tipo_sesion=='ROOT') {
		if(isset($_POST['accionEditar'])) {
			...
			if($errFlag) {
				...
				if(isset($_POST['txtCargo'])) $cargo = $conexion->real_escape_string($_POST['txtCargo']);
				else $cargo = '';
				...
				$update .= " email='".$email."', fono_agencia='".$fono_agencia."', cargo='".$cargo."' "
...
function mostrar_editar_usuario($id_usuario_sesion, $tipo_sesion, $usuario_sesion, $conexion, $errArr){
 ...
 <?php
	...
	if($num) {
		...
		if(isset($_POST['txtCargo'])) $txtCargo = $_POST['txtCargo']; else $txtCargo = $fila['cargo'];
