...
function main_navegation($link_page,$id_usuario_sesion,$tipo_sesion,$usuario_sesion,$conexion){
		...
		if($tipo_sesion!='ROOT'){
				...
				if($resup = $conexion->query($selectP,MYSQLI_STORE_RESULT)){
						$num = $resup->num_rows;
						if($num>0){
							 ...
							 echo'<ul>';
							   ...
							   if($desgravamen){    //DESGRAVAMEN
								   ...
								   echo'<li '.$data.'>
											...
											<ul '.$close.'>';
										   echo'<li><a href="?l=des_datos&var=de">Administrar parametros del Producto</a></li>
												...
												echo'<li><a href="?l=des_contenido&var=de">Administrar contenido</a></li>';
						 ...
						 echo"<div style='font-size:8pt; text-align:center; margin-top:20px; margin-bottom:15px; border:1px solid #C68A8A; background:#FFEBEA; padding:8px; width:600px;'>No existe permisos creados para este usuario</div>";	
