...
if(isset($_SESSION['usuario_sesion']) && isset($_SESSION['tipo_sesion'])) {
	...
	if(isset($_POST['username'])) {
		...
		if(validar_login($conexion)) {
			...
			header('Location: index.php?l=estados&var=est');
			exit;
		} else {
				...
				<div class="da-panel-header" style="text-align:right; padding-top:5px; padding-bottom:5px;">
					<ul class="action_user">
						<li style="margin-right:6px;">
						   <a href="?l=estados&var='.$_GET['var'].'&crear=v" class="da-tooltip-s various fancybox.ajax" title="<span lang=\'es\'>Añadir Estado</span>">
												...
												while($regi = $res->fetch_array(MYSQLI_ASSOC)){
													echo'<tr id="del-'.$regi['id_estado'].'">
															...
															<td class="da-icon-column">
															   <ul class="action_user">
																  <li style="margin-right:5px;"><a href="?l=estados&idestado='.base64_encode($regi['id_estado']).'&id_ef='.base64_encode($regief['id_ef']).'&editar=v&var='.$_GET['var'].'" class="edit da-tooltip-s" title="<span lang=\'es\'>Editar</span>"></a></li>
																  <li><a href="#" class="eliminar da-tooltip-s" title="<span lang=\'es\'>Eliminar</span>" id="'.$regi['id_estado'].'|'.$regief['id_ef'].'"></a></li>';
														  echo'</ul>	
															</td>
														</tr>';
												}
	...
	if($rs = $conexion->query($select, MYSQLI_STORE_RESULT)){
			...
			if($num>0) {
			   ...
			   echo'<div class="da-panel" style="width:600px;">
				...
				echo '<meta http-equiv="refresh" content="2; url=index.php?l=formapago&var='.$_GET['var'].'">';
			}
	}else{
		echo"<div style='font-size:8pt; text-align:center; margin-top:20px; margin-bottom:15px; border:1px solid #C68A8A; background:#FFEBEA; padding:8px; width:600px;'>Error en la consulta: "."\n ".$conexion->errno . ": " .$conexion->error."</div>";
	}
