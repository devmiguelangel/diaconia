...
class Session
{
    ...
    public function __construct()
    {
        ...
        if (strpos($domain, '.app') !== false) {
          # code...
        } elseif (strpos($domain, 'diaconia') === false) {
            $path = '/diaconia';
        }

        // $this->max_time = (60 * 60 * 8);
        $this->max_time = (60 * 10);
        session_set_cookie_params($this->max_time, $path);
    ...
    public function setSessionCookie() 
    {
        ...
        if (empty($_SESSION['idUser']) === false
            && isset($_SESSION['idEF'])) {
    ...
    private function setDataCookie()
    {
        if (isset($_SESSION['idUser'], $_SESSION['idEF'])) {
            setcookie('DiaconiaToken[user]', $_SESSION['idUser'], time() + $this->max_time, '/', '', false, true);
            setcookie('DiaconiaToken[ef]', $_SESSION['idEF'], time() + $this->max_time, '/', '', false, true);
        }
    }
    ...
    public function remove_session()
    {
        ...
        if (isset($_COOKIE['DiaconiaToken'])) {
            setcookie('DiaconiaToken', '', time() - 3600, '/', '', false, true);
            setcookie('DiaconiaToken[user]', '', time() - 3600, '/', '', false, true);
            setcookie('DiaconiaToken[ef]', '', time() - 3600, '/', '', false, true);
