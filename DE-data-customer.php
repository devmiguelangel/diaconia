    ...
    if ($swCl === false) {
        if ($nCl > 0) {
            ...
            <table class="list-cl">
                ...
                <tbody>
                <?php $cont = 1;
                foreach ($data_list as $key => $rowCl): ?>
                    <tr>
                        ...
                        <td><?=$rowCl['cl_pc'];?></td>
                        <td><a
                                href="de-quote.php?ms=<?=$_GET['ms'];?>&page=<?=$_GET['page'];?>&pr=<?=$_GET['pr'];?>&idc=<?=$_GET['idc'];?>&idCl=<?=base64_encode($rowCl['id_cliente']);?>"
                                title="Editar Información"><img src="img/edit-user-icon.png" width="40" height="40"
                                                                alt="Editar Información" title="Editar Información"></a>
                        </td>
                    </tr>
                    <?php
                    $cont++;
    ...
    -->
      ...
      <textarea id="dc-address-<?=$k?>" name="dc-address-<?=$k?>"
                class="required fbin"><?=$data['address'];?></textarea><br>
                    ...
                    <div class="content-input">
                        <select id="dc-occupation-<?=$k?>" name="dc-occupation-<?=$k?>" class="fbin occupation">
                            <option value="">Seleccione...</option>
