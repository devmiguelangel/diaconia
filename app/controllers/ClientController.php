...
class ClientController extends Diaconia
{
    ...
    public function postClient($data, &$mess)
    {
        ...
        if ($pr === 'DE|02') {
            ...
            if ($record === 0) {
                $mess[2] = 'Error. Debe registrar al menos un Cliente.';
                ...
                for ($k = 0; $k < $nCl; $k++) {
                    if (isset( $data['dc-name-' . $k], $data['dc-doc-id-' . $k] )) {
                        ...
                        if (empty( $aux_occupation ) === false) {
                            $temp_data['occupation'] = '"' . $this->cx->real_escape_string(trim(base64_decode($aux_occupation))) . '"';
                        }
                        ...
                        if (isset( $data['dc-amount-' . $k] )) {
                            $temp_data['amount'] = $this->cx->real_escape_string(trim(base64_decode($data['dc-amount-' . $k])));
                        }
                        ...
                        if (( $temp_data['status'] !== 'CAS' && $temp_data['status'] !== 'VIU' ) || $temp_data['gender'] !== 'F') {
                            $temp_data['married'] = '';
                        }
                        ...
                        if ($this->verifyYearUser($data['data']['edad_min'], $data['data']['edad_max'],
                                $temp_data['birth']) === false
                        ) {

                            $name       = $temp_data['name'] . ' ' . $temp_data['patern'] . ' ' . $temp_data['matern'];
                            $birth_flag = false;
                            $birth_mess .= 'La Fecha de nacimiento del titular ' . $name . ' no esta en el rango permitido de edades <br>';
                        }
                    }
                }
                ...
                if ($birth_flag) {
                    ...
                    if ($flag) {
                        ...
                        if ($ClientRepo->putClientData($data, $arr_cl[0], $bc)) {
                            $mess[0] = 1;
                            $mess[1] = 'de-quote.php?ms=' . $ms . '&page=' . $page . '&pr=' . $pr . '&idc=' . base64_encode($data['idc']);
                    ...
                    $mess[2] = $birth_mess . '[ ' . $data['data']['edad_min'] . ' - ' . $data['data']['edad_max'] . ' ]';
                }
            } else {
                $mess[2] = 'Error .';
            }
